...
class LabelDialog(QDialog):
    ...
    def __init__(self, text="Enter object label", parent=None, list_item=None):
        ...
        # 添加自动补全功能
        self.label_history = self.loadLabelHistory()
        completer = QCompleter(self.label_history)
        completer.setCaseSensitivity(Qt.CaseInsensitive)
        self.edit.setCompleter(completer)
        
        # 以下代码需要正确缩进到 __init__ 方法中
        layout = QVBoxLayout()
    ...
    def loadLabelHistory(self):
        # 从配置文件加载历史标签
        try:
            with open('label_history.txt', 'r', encoding='utf-8') as f:
                return [line.strip() for line in f.readlines()]
        except:
            return ['person', 'car', 'bicycle', 'dog', 'cat']  # 默认标签
